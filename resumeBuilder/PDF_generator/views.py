...
def resumeview(request):
    ...
    if request.method == 'GET':
        return render(request,'output2.html', context)  # , context_instance=RequestContext(request))
...
def resumeview(request):
    ...
    if request.method == 'GET':
        return render(request,'cv.html', context)  # , context_instance=RequestContext(request))
...
class GeneratePDF(View):
    def get(self, request, *args, **kwargs):
        ...
        html = template.render(context)
        pdf = render_to_pdf('cv.html', context)
        if pdf:
            response = HttpResponse(pdf, content_type='/pdf')
            filename = "Resume_%s.pdf" %("12341231")
            content = "inline; filename='%s'" %(filename)
